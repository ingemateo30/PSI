                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {internetPlans.map((plan, index) => (
                            <div
                                ...
                                className={`relative group cursor-pointer rounded-3xl transition-all duration-500 transform hover:scale-105 ${plan.featured
                                        ? "ring-4 ring-[#e31e25] ring-opacity-70 shadow-2xl scale-105 z-10"
                                        : "shadow-xl hover:shadow-2xl"
                                    }`}
                            >
                               

                                <div className={`flex flex-col h-full px-8 pt-8 pb-6 rounded-b-3xl ${plan.featured
                                        ? "bg-gradient-to-br from-[#0e6493] to-[#073a57] text-white"
                                        : "bg-white"
                                    }`}>
                                    {/* Speed highlight */}
                                    <div className="mb-6 text-center">
                                        <div className={`text-7xl font-extrabold leading-none ${plan.featured ? "text-white" : "text-[#e31e25]"
                                            }`}>
                                            {plan.speed}
                                        </div>
                                        <div className={`text-xl font-medium ${plan.featured ? "text-blue-100" : "text-[#0e6493]"
                                            }`}>
                                            Megas
                                        </div>
                                    </div>
                                    ...
                                    <div className="text-center mb-8">
                                        <h3 className={`text-2xl font-bold mb-3 ${plan.featured ? "text-white" : "text-[#0e6493]"
                                            }`}>
                                            {plan.name}
                                        </h3>
                                        <div className="flex justify-center items-baseline">
                                            <span className={`text-4xl font-bold ${plan.featured ? "text-white" : "text-[#e31e25]"
                                                }`}>
                                                {plan.price}
                                            </span>
                                            <span className={`text-sm ml-2 ${plan.featured ? "text-blue-100" : "text-gray-500"
                                                }`}>
                                                /mes
                                            </span>
                                        </div>
                                        <p className={`text-sm mt-1 ${plan.featured ? "text-gray-200" : "text-gray-600"}`}>
                                            IVA Incluido
                                        </p>
                                    </div>

                                    {/* Speed details */}
                                    <div className={`flex justify-between p-4 mb-6 rounded-xl ${plan.featured ? "bg-white/10" : "bg-blue-50"
                                        }`}>
                                        <div className="text-center">
                                            <div className="flex justify-center">
                                                <Download className={`h-5 w-5 ${plan.featured ? "text-blue-100" : "text-[#0e6493]"
                                                    }`} />
                                            </div>
                                            <div className={`font-bold text-lg ${plan.featured ? "text-white" : "text-[#0e6493]"
                                                }`}>
                                                {plan.speed} MB
                                            </div>
                                            <div className={`text-xs ${plan.featured ? "text-blue-100" : "text-gray-500"
                                                }`}>
                                                Descarga
                                            </div>
                                        </div>

                                        <div className={`h-auto w-px ${plan.featured ? "bg-white/20" : "bg-gray-300"
                                            }`}></div>

                                        <div className="text-center">
                                            <div className="flex justify-center">
                                                <Upload className={`h-5 w-5 ${plan.featured ? "text-blue-100" : "text-[#0e6493]"
                                                    }`} />
                                            </div>
                                            <div className={`font-bold text-lg ${plan.featured ? "text-white" : "text-[#0e6493]"
                                                }`}>
                                        ...
                                        <ul className="space-y-3">
                                            {plan.benefits.map((benefit, i) => (
                                                <li key={i} className="flex items-center">
                                                    <CheckCircle className={`h-4 w-4 mr-2 ${plan.featured ? "text-blue-100" : "text-[#0e6493]"
                                                        }`} />
                                                    <span className={`text-sm ${plan.featured ? "text-blue-50" : "text-gray-600"
                                                        }`}>
                                                        {benefit}
                                                    </span>
                                                </li>
                                    ...
                                    {/* Button */}
                                    <button className={`mt-8 w-full py-3 rounded-xl font-bold text-base flex items-center justify-center space-x-2 transition-all duration-300 ${plan.featured
                                            ? "bg-white text-[#0e6493] hover:bg-blue-50"
                                            : "bg-gradient-to-r from-[#0e6493] to-[#073a57] text-white hover:shadow-lg"
                                        }`}>
                                    ...
                                    {plan.featured && (
                                        <div className="absolute -top-3 -right-3 bg-[#e31e25] text-white text-xs font-bold p-2 rounded-full shadow-lg flex items-center justify-center">
